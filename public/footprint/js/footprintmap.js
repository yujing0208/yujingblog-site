...
(function () {
    ...
    const Utils = {
        ...
        sanitizeLocation: (item, index) => {
            const coords = Utils.parseCoords(item.coordinates || item.coordinate || item.coords || item.position || (item.lat != null && item.lng != null ? `${item.lng},${item.lat}` : null));
            if (!coords) return null;
            // markerColor 兼容教程字段名（sunset/ocean/violet/forest/amber/citrus 或任意 CSS 颜色）
            const preset = item.markerPreset || item.markerColor || CONFIG.MARKER_PRESETS[index % CONFIG.MARKER_PRESETS.length];
            ...
            return {
                ...
                markerPreset: preset,
                markerStyle: CONFIG.MARKER_STYLES[preset] ? `background:${CONFIG.MARKER_STYLES[preset]}` : (typeof preset === 'string' && /^#|rgb|hsl/.test(preset) ? `background:${preset}` : '')
            };
        },
    ...
    class AMapEngine {
        ...
        async load() {
            ...
            return new Promise((resolve, reject) => {
                ...
                const sec = (this.container && this.container.dataset.amapSecurity) || window.__AMAP_SECURITY_CODE__ || '';
                s.onload = () => { window._AMapSecurityConfig = { securityJsCode: sec }; resolve(); };
                s.onerror = reject;
        ...
        init(locations) {
            ...
            const initialCenter = locations.length ? [locations[0].lng, locations[0].lat] : [104, 35];
                    ...
                    AMap.plugin('AMap.Geolocation', () => {
                        ...
                        geo.getCurrentPosition((status, result) => {
                            if (status === 'complete' && result.position) {
                                ...
                                this.map.setZoomAndCenter(14, pos);
                                if (this._locMarker) this.map.remove(this._locMarker);
                                this._locMarker = new AMap.Marker({
                                    position: pos,
                                    content: '<div class="fp-loc-marker" title="我的位置"></div>',
                                    offset: new AMap.Pixel(-9, -9),
                                    zIndex: 200,
                                    map: this.map
                                });
                            } else {
                                console.warn('[FootprintMap] 定位失败：', result && result.message);
                            }
                            resolve();
                        });
                    });
    ...
    // --- 主加载流程 ---
    // 数据来源优先级：
    //   1) 容器 data-footprints 内联 JSON（构建时由页面注入，无需额外请求，推荐）
    //   2) 容器 data-json 指向的 URL（兼容旧的 /footprint/data/footprints.json 方式）
    async function initMap(container) {
        const { footprints, json: dataUrl, amapKey: apiKey } = container.dataset;
        if (!apiKey) return container.innerHTML = `<div class="footprint-map__error">配置错误：缺少 API Key</div>`;
        try {
            let list;
            if (footprints) {
                const raw = JSON.parse(footprints);
                list = (Array.isArray(raw) ? raw : raw.locations || []).map(Utils.sanitizeLocation).filter(Boolean);
            } else if (dataUrl) {
                const raw = await (await fetch(dataUrl)).json();
                list = (raw.locations || raw).map(Utils.sanitizeLocation).filter(Boolean);
            } else {
                list = [];
            }
            ...
            if (container.dataset.locate === 'auto') controls.locate();
        } catch (e) {
    ...
    function renderUI(container, allData, controls) {
        ...
        addBtn(icons.reset, () => controls.fitView()); addBtn(icons.plus, () => controls.zoomIn()); addBtn(icons.minus, () => controls.zoomOut());
        const locIcon = '<svg viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5a2.5 2.5 0 110-5 2.5 2.5 0 010 5z"/></svg>';
        addBtn(locIcon, () => controls.locate());
